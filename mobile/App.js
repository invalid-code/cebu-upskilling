...
function JobsScreen() { return <ListScreen title="Find work" endpoint="/jobs" empty="No matching jobs yet." renderItem={(item, index) => <Card key={item.jobId || index}><Text style={styles.cardEyebrow}>{item.location || 'CEBU'}</Text><Text style={styles.cardTitle}>{item.title || item.jobTitle}</Text><Text style={styles.cardText}>{item.companyName || item.company || 'Local employer'}</Text><Button secondary onPress={() => {}}>View role</Button></Card>} />; }
...
function AccountScreen() { const { user, logout } = useAuth(); return <Screen><Header eyebrow="ACCOUNT" title="Your profile" subtitle="Keep your pathway details up to date." /><Card><Text style={styles.cardTitle}>{user?.firstName} {user?.lastName}</Text><Text style={styles.cardText}>{user?.emailAddress || user?.email}</Text><Text style={styles.cardText}>Target role: {user?.targetRole || 'Not set'}</Text></Card><Button secondary onPress={logout}>Sign out</Button></Screen>; }
