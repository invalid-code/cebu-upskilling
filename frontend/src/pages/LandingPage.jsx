...
const HERO_C = 2 * Math.PI * 45;

function levelWord(v) {
  return v >= 80 ? 'Advanced' : v >= 55 ? 'Intermediate' : v >= 35 ? 'Beginner' : 'Not started';
}
...
function getBand(v) {
  return BANDS.find((x) => v >= x.lo) ?? BANDS[BANDS.length - 1];
}
...
export default function LandingPage() {
  ...
  const [displayedAiPct, setDisplayedAiPct] = useState(ROLES[0].match);
  ...
  // Animate AI pct when role changes
  useEffect(() => {
    const rafIds = rafIdsRef.current;
    const reduce = typeof window.matchMedia === 'function' && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const target = ROLES[selectedRole].match;
    if (reduce) {
      setDisplayedAiPct(target);
      return;
    }
    let rafId = null;
    let start = null;
    const from = displayedAiPct;
    const animate = (t) => {
      if (start === null) start = t;
      const p = Math.min((t - start) / 700, 1);
      const e = 1 - Math.pow(1 - p, 4);
      setDisplayedAiPct(Math.round(from + (target - from) * e));
      if (p < 1) {
        rafId = trackRaf(requestAnimationFrame(animate));
      }
    };
    if (from !== target) {
      rafId = trackRaf(requestAnimationFrame(animate));
  ...
  useEffect(() => {
    ...
    let start = null;
    const target = 72;
    const delayId = trackTimeout(setTimeout(() => {
      const step = (t) => {
        if (start === null) start = t;
        const p = Math.min((t - start) / 1500, 1);
        const e = 1 - Math.pow(1 - p, 4);
        setHeroPct(Math.round(target * e));
        if (p < 1) rafId = trackRaf(requestAnimationFrame(step));
      };
      rafId = trackRaf(requestAnimationFrame(step));
    }, 250));
    return () => {
      clearTimeout(delayId);
      timeoutIds.delete(delayId);
      if (rafId !== null) {
        cancelAnimationFrame(rafId);
        rafIds.delete(rafId);
      }
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      setNavStuck(stuck);
      if (navEl) navEl.classList.toggle('stuck', stuck);
    };
    onScroll();
    ...
    if (drawerEl) drawerEl.addEventListener('click', onDrawerClick);
    ...
    function runCount(el) {
      ...
      const unitEl = el.querySelector('i');
      const unit = unitEl ? unitEl.outerHTML : '';
      if (reduce) {
        el.innerHTML = fmt(end, dec) + unit;
        return;
      }
      let t0 = null;
      const dur = 1300;
    ...
    if ('IntersectionObserver' in window && !reduce) {
      ...
      document.querySelectorAll('.landing-page .rv, .landing-page .path, .landing-page #heroPanel, .landing-page .stats').forEach((el) => obs.observe(el));
    } else {
    ...
    return () => {
      ...
      if (drawerEl) drawerEl.removeEventListener('click', onDrawerClick);
  ...
  return (
    <div className="landing-page">
      <header className={`nav${navStuck ? ' stuck' : ''}${navOpen ? ' open' : ''}`} id="nav">
        <div className="shell nav__in">
          ...
          <nav className="navlinks" aria-label="Main">
            <a href="#product" aria-current={activeHref === '#product' ? 'true' : undefined}>Product</a>
            <a href="#pathway" aria-current={activeHref === '#pathway' ? 'true' : undefined}>How It Works</a>
            <a href="#employers" aria-current={activeHref === '#employers' ? 'true' : undefined}>For Employers</a>
            <a href="#providers" aria-current={activeHref === '#providers' ? 'true' : undefined}>For Training Providers</a>
            <a href="#about" aria-current={activeHref === '#about' ? 'true' : undefined}>About</a>
            <a href="#pilot" aria-current={activeHref === '#pilot' ? 'true' : undefined}>Pilot</a>
          </nav>
        ...
        <div className={`drawer${navOpen ? ' show' : ''}`} id="drawer">
          <ul>
            <li><a href="#product" onClick={closeDrawer}>Product <i>01</i></a></li>
            <li><a href="#pathway" onClick={closeDrawer}>How It Works <i>02</i></a></li>
            <li><a href="#ai" onClick={closeDrawer}>AI Career Matching <i>03</i></a></li>
            <li><a href="#employers" onClick={closeDrawer}>For Employers <i>04</i></a></li>
            <li><a href="#providers" onClick={closeDrawer}>For Training Providers <i>05</i></a></li>
          ...
          <div style={{ padding: '16px clamp(20px, 5vw, 56px) 20px', borderTop: '1px solid var(--line)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {user ? (
              ...
              <>
                ...
                <Link className="btn btn--ghost" to="/login" onClick={closeDrawer} style={{ justifyContent: 'center' }}>Sign in</Link>
              </>
            )}
          </div>
        </div>
                ...
                <div className="dash__top">
                  ...
                  <div className="ring">
                    <svg viewBox="0 0 104 104" aria-hidden="true">
                      ...
                      <circle className="val" id="heroRing" cx="52" cy="52" r="45" style={{ strokeDashoffset: String(HERO_C - HERO_C * 72 / 100) }} />
                    </svg>
                    <span className="ring__label"><b id="heroPct">{heroPct}%</b><span>Job match</span></span>
                  </div>
                </div>
                ...
                <div className="roles rv" style={{ '--i': '3' }} role="group" aria-label="Choose a target role">
                  {ROLES.map((r, idx) => (
                    <button
                      ...
                      className="rolebtn"
                      data-role={idx}
                      aria-pressed={selectedRole === idx ? 'true' : 'false'}
              ...
              <div className="panel panel--dark rv" style={{ '--i': '2' }}>
                ...
                <div className="aip">
                  <div className="aip__row">
                    <h5>Target</h5>
                    <strong id="aiRole" style={{ fontSize: '1.19rem', fontWeight: '700', letterSpacing: '-.022em', display: 'block' }}>{activeRole.name}</strong>
                    <span className="tiny" id="aiCtx" style={{ color: 'var(--d-muted)' }}>{activeRole.ctx}</span>
                  </div>
                  <div className="aip__row">
                    <h5>Current capability</h5>
                    <div id="aiCur">
                      {activeRole.cur.map(([label, pct, lvl]) => (
                        <div className="lvl" key={label}>
                          <span>{label}</span>
                          <span className="lvl__t"><i className={lvl} style={{ width: `${pct}%` }}></i></span>
                          <span className="lvl__p">{levelWord(pct)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className="aip__row">
                    <h5>Missing</h5>
                    <div className="chips" id="aiGaps">
                      {activeRole.gaps.map((g) => (
                  ...
                  <div className="aip__row aip__out">
                    <div>
                      ...
                      <ol className="plan" id="aiPlan">
                        {activeRole.plan.map((p) => (
                          <li key={p}>{p}</li>
                        ))}
                      </ol>
                    </div>
                    <div className="miniring">
                      <svg width="88" height="88" viewBox="0 0 88 88" aria-hidden="true">
                        ...
                        <circle className="val" id="aiRing" cx="44" cy="44" r="38" style={{ strokeDashoffset: String(AC - AC * activeRole.match / 100) }} />
                      </svg>
                      <span className="miniring__l"><b id="aiPct">{displayedAiPct}%</b><span>Job match</span></span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="score__grid">
              <div className="panel gauge rv">
                <div className="gauge__val">
                  <b className="num" id="gVal">{score}<sup>%</sup></b>
                  <span className="gauge__band"><strong id="gBand">{band.name}</strong><span id="gNext">{bandIdx === 0 ? 'top band reached' : `next band at ${BANDS[bandIdx - 1].lo}%`}</span></span>
                </div>
                <div className="scale">
                  <div className="scale__bars" id="gBars">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <i key={i} className={`${i < filled ? 'on' : ''} ${i === 4 && filled === 5 ? ' top' : ''}`.trim()}></i>
                    ))}
                  </div>
                ...
                <input type="range" id="gRange" min="0" max="100" value={score} step="1" onChange={(e) => setScore(parseInt(e.target.value, 10))} aria-label="Job Match Score simulator" />
                <p className="gauge__msg" id="gMsg">{band.msg}</p>
              </div>

              <div>
                <ul className="bandlist" id="bandList">
                  {BANDS.map((b) => (
                    <li key={b.lo} data-lo={b.lo} className={band.lo === b.lo ? 'act' : ''}>
                      <span className="r">{b.lo === 85 ? '85–100' : b.lo === 65 ? '65–84' : b.lo === 45 ? '45–64' : b.lo === 25 ? '25–44' : '0–24'}</span>
                      <span><span className="n">{b.name}</span><span className="tiny">{b.lo === 85 ? 'Apply now. Your capability covers the role\'s core requirements.' : b.lo === 65 ? 'Worth applying while you close the remaining gaps.' : b.lo === 45 ? 'Foundation is there. Two or three targeted courses will move you up.' : b.lo === 25 ? 'Start with the fundamentals in the recommended order.' : 'A different starting role may get you there faster. We\'ll suggest one.'}</span></span>
                    </li>
                  ))}
                </ul>
              ...
              <div className="panel">
                ...
                <div className="panel__body">
                  ...
                  <label className="toggle" style={{ marginTop: '20px' }}>
                    <input type="checkbox" id="skillMode" checked={skillView} onChange={(e) => setSkillView(e.target.checked)} />
                    <span className="tr" aria-hidden="true"></span>
                    <span id="skillModeLabel">{skillView ? 'Skill-based view' : 'Résumé filter view'}</span>
                  </label>
                  <div className="cands" id="candList">
                    ...
                    <div className={`cand${!skillView ? ' hidden' : ''}`} data-degree="0" data-yrs="0">
                    ...
                    <div className={`cand${!skillView ? ' hidden' : ''}`} data-degree="1" data-yrs="0">
                  ...
                  <p className="empnote" id="empNote">
                    {skillView ? (
            ...
            <div style={{ marginTop: 'clamp(48px,6vw,80px)' }}>
              ...
              <div className="team">
                ...
                <div className="mate rv" style={{ '--i': '1' }}><img className="mate__photo"
                  src={'/images/MEMBERS/evangelista-jess-matthew.jpg'} alt="Jess Matthew Evangelista" /><b>Jess Matthew
                    Evangelista</b><span>Co-founder</span></div>
                <div className="mate rv" style={{ '--i': '2' }}><img className="mate__photo"
                  src={'/images/MEMBERS/sucgang-jake-fiel.jpg'} alt="Jake Sucgang" /><b>Jake
